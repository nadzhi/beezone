...
@section('content')
        ...
        <div class="container">
            ...
            <div class="product-wrapper">
                ...
                <div class="row no-gutters cols-xs-space cols-sm-space cols-md-space">
                    <div class="col-lg-6">
                        <div class="product-gal d-flex flex-row-reverse">
                            @if(is_array(json_decode($product->photos)) && count(json_decode($product->photos)) > 0)
                                <div class="product-gal-img bg-white">
                                    <img class="xzoom img-fluid" 'http://test.beezone.kz/public/{{ $product->featured_img }}'
										 src="http://test.beezone.kz/public/{{ json_decode($product->photos)[0] }}" 
										 xoriginal="http://test.beezone.kz/public/{{ json_decode($product->photos)[0] }}" />
                                </div>
                                <div class="product-gal-thumb">
                                    <div class="xzoom-thumbs">
                                        @foreach (json_decode($product->photos) as $key => $photo)
                                            <a class="xzoom-link" href="{{ asset($photo) }}">
                                                <img class="xzoom-gallery" width="80" src="http://test.beezone.kz/public/{{ $photo }}"  
													 @if($key == 0) xpreview="http://test.beezone.kz/public/{{ $photo }}" @endif>
                                            </a>
                                        @endforeach
                                    </div>
                                </div>
                        ...
                        <div class="product-desc-tab bg-white mt-4">
                    ...
                    <div class="col-lg-6">
                        <!-- Product description -->
                        <div class="product-description-wrapper bg-white">
                            <!-- Product title -->
                            <h2 class="product-title pb-2">
                                {{ __($product->name) }}
                            </h2>
                            <hr class="mt-2">
                            <div class="row">
                                <div class="col-6">
                                    ...
                                    <div class="rating ">
                                        ...
                                        <span class="rating-count">({{__('customer reviews')}} {{ $total }})</span>
                                    </div>
                            ...
                            @if(home_price($product->id) != home_discounted_price($product->id))
                                ...
                                <div class="row no-gutters mt-4">
                                    <div class="col-5">
                                        <div class="product-description-label">Розничная цена с доставкой:</div>
                                    </div>
                                    <div class="col-7">
                                ...
                                <div class="row no-gutters mt-3">
                                    <div class="col-5">
                                        <div class="product-description-label mt-1">Розничная цена без доставки:</div>
                                    </div>
                                    <div class="col-7">
                                        <div class="product-price">
                                            <strong>
                            ...
                            <form id="option-choice-form">
                                ...
                                <input type="hidden" name="quantity" class="form-control input-number text-center" placeholder="1" value="1" min="1">
                            </form>
                            ...
                            <div class="d-table width-100 mt-3">
                                <div class="d-table-cell">
                                    ...
                                    @endif
                                    <!-- Add to cart button -->
                                    <button style="float:left" type="button" class="btn btn-styled btn-alt-base-1 c-white btn-icon-left strong-700 hov-bounce hov-shaddow ml-2" onclick="addToCart()">
                                        <i class="la la-shopping-cart"></i>
                                        <span class="d-none d-md-inline-block"> {{__('Add to cart')}}</span>
                                    </button>

                                    <!--button style="float:left"  type="button" class="btn btn-styled btn-base-1 btn-icon-left strong-700 hov-bounce hov-shaddow" onclick="buyNow()">
                                        <i style="font-size: 20px;" class="la la-shopping-cart"></i> Купить
                                    </button-->

									<h3 style="display:inline-block;margin-left:16px;cursor:pointer;float:left" class="btn-icon-left prod-add-wishlist" onclick="addToWishList({{ $product->id }})">
							...
							<hr class="mt-3 mb-0">
                            <?php 
                                $product_price = DB::table("product_prices")->where('product_id', $product->id)->get(); 
                            ...
                            ?>
                                ...
                                <table class="table">
                                    ...
                                    <tbody>
                                        <?php $counter = 0; foreach ($product_price as $price): $counter++; ?>
                                            <tr>
                                                <td><?=$counter?></td>
                                                <td>от <?=$price->count?> {{ $product->unit }}</td>
                                                <td><?=$price->price?>₸</td>
                                                <td><?=($price->price*$price->count)?>₸</td>
                                                <td>
                                                    <form id="option-choice-form-<?=$price->level?>">
                                                        ...
                                                        <button onclick="addSelect(<?=$price->level?>)" type="button" class="btn btn-styled btn-base-1 btn-icon-left strong-700 hov-bounce hov-shaddow">Выбрать</button>
                                                    </form>
                                                </td>
                                            </tr>
                                        <?php endforeach; ?>
                            ...
                            <?php else: ?>
                                <p class="pt-3 text-center">Оптовые цены отсутствует</p>
                            <?php endif; ?>
					...
					<div class="seller-info-box bg-white mb-3 mt-4" style="border:1px solid #fff">
    ...
    <section class="gry-bg">
        <div class="container">
            <div class="row product-wrapper" style="max-width: 1240px">
                <div class="col-xl-12">
                                    ...
                                    <div class="row no-gutters product-box-2 align-items-center">
                                        <div class="col-4">
                                            <div class="position-relative overflow-hidden h-100">
                                                <a href="{{ route('product', $related_product->slug) }}" class="d-block product-image h-100" style="background-image:url('http://test.beezone.kz/public/{{ $related_product->thumbnail_img }}');">
                                                </a>
                                            </div>
                                        </div>
