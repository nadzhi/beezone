                            ...
                            <div class="form-box bg-white mt-4">
                                ...
                                <div class="form-box-content p-3">
                                    <div class="row">
										...
										<div class="col-md-3">
                                            <div class="form-control mb-3 c-pointer" data-toggle="modal" data-target="#productSelectModal" id="product_select">Выбрать товар</div>
                                        </div>
                                    </div>
                                    <div class="row">
                                        <div class="col-md-2">
                                    ...
                                    <div class="row">
                                        ...
                                        <div class="col-md-10">
                                            <div class="mb-3">
                                                <select class="form-control mb-3 selectpicker" data-placeholder="Выбрать бренд" id="brands" name="brand_id" required>
                                    ...
                                    <div class="row">
                                        ...
                                        <div class="col-md-10">
                                            <input type="text" class="form-control mb-3" name="unit" placeholder="Количество товара" required>
                                        </div>
                                    </div>
                                    <div class="row">
                                        <div class="col-md-2">
                                        ...
                                        <div class="col-md-10">
                                            <input type="text" class="form-control mb-3 tagsInput" name="tags[]" placeholder="Введите и нажмите ввод" data-role="tagsinput" required>
                                        </div>
                                    </div>
                                </div>
                            </div>
...
<!-- Modal -->
    <div class="modal fade" id="productSelectModal" tabindex="-1" role="dialog" aria-labelledby="exampleModalLabel" aria-hidden="true" data-backdrop="static">
        <div class="modal-dialog modal-lg" role="document">
            <div class="modal-content">
                <div class="modal-header">
                    <h6 class="modal-title" id="exampleModalLabel">Выбрать товар</h6>
                    <button type="button" class="close" data-dismiss="modal" aria-label="Close">
                    <span aria-hidden="true">&times;</span>
                    </button>
                </div>
                <div class="modal-body">
                    <div class="row no-gutters modal-categories mt-4 mb-2">
                        <div class="col-4">
                            <div class="modal-category-box c-scrollbar">
                                ...
                                <div class="modal-category-list has-right-arrow">
                                    <ul id="categories">
                                        @foreach ($categories as $key => $category)
                                            <li onclick="get_products_by_category(this, {{ $category->id }})">{{ __($category->name) }}</li>
                                        @endforeach
                                    </ul>
                                </div>
                            </div>
                        ...
                        <div class="col-8">
                            <div class="modal-category-box c-scrollbar" id="products_list">

                                <div class="modal-category-list has-right-arrow">
                                    <div id="ajax_products"  class="row">
                ...
                <div class="modal-footer">
                    <button type="button" class="btn btn-secondary" data-dismiss="modal">{{__('cancel')}}</button>
                    <button type="button" class="btn btn-primary" onclick="closeModalProducts()">{{__('Confirm')}}</button>
                </div>
            </div>
        </div>
    </div>
...
@section('script')
    <script type="text/javascript">
        ...
        $(document).ready(function(){
			...
			$("body").on("click",".product-popup-cls",function() {
				$(".product-popup-cls").removeClass("products-active-class");
				$(this).addClass("products-active-class");
			});
        });
		...
		function get_products_by_category(el, cat_id){
            list_item_highlight(el);
            category_id = cat_id;
            subcategory_id = null;
            subsubcategory_id = null;
            category_name = $(el).html();
            $('#ajax_products').html(null);
            $.post('{{ route('subcategories.get_products_by_category') }}',{_token:'{{ csrf_token() }}', category_id:category_id}, function(data){
                for (var i = 0; i < data.length; i++) {
                    $('#ajax_products')
					.append(
						'<div onclick="confirm_product(this, '+data[i].id+')" class="col-xxl-6 col-xl-6 col-lg-6 col-md-6 col-6 product-popup-cls">'+
							'<div class="product-box-2 bg-white alt-box my-2">'+
								'<div class="position-relative overflow-hidden">'+
									'<a class="d-block product-image h-100" style="background-image:url(http://test.beezone.kz/public/' + data[i].featured_img +');" tabindex="0">'+
									'</a>'+
								'</div>'+
		...
		function confirm_product(el, prod_id){
            list_item_highlight(el);
            product_id = prod_id;
    	}
		...
		function closeModalProducts(){
			if(product_id > 0){
				$.post('{{ route('subcategories.add_product') }}',{_token:'{{ csrf_token() }}', product_id:product_id}, function(data){
					if(data == "0"){
    	...
    	function add_more_customer_choice_option(){
    		$('#customer_choice_options').append('<div class="row mb-3"><div class="col-8 col-md-3 order-1 order-md-0"><input type="hidden" name="choice_no[]" value="'+i+'"><input type="text" class="form-control" name="choice[]" value="" placeholder="Выбор названия"></div><div class="col-12 col-md-7 col-xl-8 order-3 order-md-0 mt-2 mt-md-0"><input type="text" class="form-control tagsInput" name="choice_options_'+i+'[]" placeholder="Введите значения выбора" onchange="update_sku()"></div><div class="col-4 col-xl-1 col-md-2 order-2 order-md-0 text-right"><button type="button" onclick="delete_row(this)" class="btn btn-link btn-icon text-danger"><i class="fa fa-trash-o"></i></button></div></div>');
    		i++;
