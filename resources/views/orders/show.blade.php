            ...
            <div class="row">
                ...
                <div class="col-lg-3">
                    ...
                    <select class="form-control demo-select2"  data-minimum-results-for-search="Infinity" id="update_delivery_status">
                        <option value="pending" @if ($delivery_status == 'pending') selected @endif>В ожидании</option>
                        <option value="on_review" @if ($delivery_status == 'on_review') selected @endif>На рассмотрении</option>
                        <option value="on_delivery" @if ($delivery_status == 'on_delivery') selected @endif>Отправлен на склад</option>
                        <option value="delivered" @if ($delivery_status == 'delivered') selected @endif>Отгружен</option>
                    </select>
                </div>
            </div>
            <hr>
    ...
    <script type="text/javascript">
        $('#update_delivery_status').on('change', function(){
            ...
            $.post('{{ route('orders.update_delivery_status') }}', {_token:'{{ @csrf_token() }}',order_id:order_id,status:status}, function(data){
                showAlert('success', 'Статус доставки обновлен');
            });
        });
        ...
        $('#update_payment_status').on('change', function(){
            ...
            $.post('{{ route('orders.update_payment_status') }}', {_token:'{{ @csrf_token() }}',order_id:order_id,status:status}, function(data){
                showAlert('success', 'Статус платежа обновлен');
            });
        });
    </script>
@endsection
